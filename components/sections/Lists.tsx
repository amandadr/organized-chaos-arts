...
export function FeatureList({
  ...
  const fills =
    palette === "deep"
      ? deepFillSequence(items.length, surface)
      : quietFillSequence(items.length, surface);

  return (
    <ol className="grid grid-cols-1 gap-6 md:grid-cols-3">
      {items.map((item, index) => (
        <li key={item.title} className={`oca-color-card md:p-8 ${fills[index]}`}>
  ...
  return (
    <div className="grid gap-3">
      {items.map((item, index) => (
        <details
          key={item.question}
          className={`group border-[3px] border-[color:var(--color-ink)] px-5 py-2 shadow-[4px_4px_0_var(--color-ink)] ${fills[index]}`}
        >
